...
if [ ! -d "$TARGET" ]; then
    >&2 echo "onething-clip install.sh: The install target directory, $TARGET, does not exist"
    exit 3
fi
...
if [ -z "$FILENAME" ]; then
    ...
    MINGW*)
        ...
        ;;
    *)
        echo "onething-clip install.sh: Cannot determine what binary to download for your kernel: $KERNEL"
        exit 3
        ;;
    esac
    ...
    *)
    ...
    FILENAME="onething-clip-$OS-$CPU$LIBC.$EXT"
fi

if [ -z "$TAG" ]; then
    URL="https://github.com/intellis-team/onething-clip/releases/latest/download/$FILENAME"
else
    URL="https://github.com/intellis-team/onething-clip/releases/download/$TAG/$FILENAME"
fi
...
if echo "$FILENAME" | grep "\\.tar\\.gz$"; then
    tar -xzf "$LOCAL_FILE" onething-clip
else
    unzip "$LOCAL_FILE"
fi
...
echo ""
echo "onething-clip install.sh: onething-clip has been installed to $TARGET."

set +e
TARGET_IS_IN_PATH=$(echo ":$PATH:" | grep --extended-regexp ":$TARGET:" 2>/dev/null)
if [ -z "$TARGET_IS_IN_PATH" ]; then
    echo "onething-clip install.sh: It looks like $TARGET is not in your PATH. You may want to add it to use onething-clip."
fi
